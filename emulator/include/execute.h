...
int8_t deRefPtr(int8_t regNo, CPU *cpu){ // dereference given pointer
    uint8_t memLocation=cpu->gpRegs[regNo];
    if (memLocation > RAM_SIZE/2 && memLocation < RAM_SIZE) { // if mem location is within bounds
        return cpu->ram[memLocation][0];
    } 
    cpu->errorFlag = true; printError("\nERROR: MEMORY DESTINATION OUT OF BOUNDS\n"); 
    return 0;
}
int8_t convertToInt(char* operand, CPU *cpu){ // handles binary and hex numbers
    if (operand[0] =='*'){ // " * " symbolize memory locations in ram
        if ((operand[1] == '0') && (operand[2] == 'x' || operand[2] == 'X')) {
            ...
            long memLocation = strtol(operand+2,NULL,10);
            return (int8_t)memLocation;
        } else {
        ...
        if ((operand[0] == '0') && (operand[1] == 'x' || operand[1] == 'X')) {
            ...
            long regNo = strtol(operand+1,NULL,10);
            return (uint8_t)regNo;
        } else {
...
void loadInst(DecodedInst *inst, CPU *cpu){ // take thing from ram and store in reg
    int8_t reg = convertToInt(inst->operand1,cpu); // reg number the input is moved to 
    int8_t input = convertToInt(inst->operand2,cpu);

    cpu->gpRegs[reg] = (inst->operand2[0] == '*') ? deRefPtr(input,cpu) : input; // if ptr, dereference
}
...
void storeInst(DecodedInst *inst, CPU *cpu){ // take thing from reg and store in ram
    ...
    int8_t input = convertToInt(inst->operand2,cpu); // value to be moved

    if (inst->operand1[0] == '*' && inst->operand1[1] == 'r') destination = deRefPtr(destination,cpu); // deref ptr if needed
    cpu->ram[destination][0] = input; // move to second half of ram
}

void movInst(DecodedInst *inst, CPU *cpu){ // copy value in one reg into another
    uint8_t destination = convertToInt(inst->operand1,cpu);
    uint8_t location = convertToInt(inst->operand2,cpu);
...
void arithmeticInst(DecodedInst *inst, CPU *cpu){  // arithmetic and logical ops
    ...
    int8_t inputA = convertToInt(inst->operand2,cpu);
    int8_t inputB = convertToInt(inst->operand3,cpu);
    uint8_t regOut = convertToInt(inst->operand1,cpu);

    if (inst->operand3[0] =='r') inputB = cpu->gpRegs[inputB]; // get value in reg if wanted
    if (inst->operand2[0] =='r') inputA = cpu->gpRegs[inputA]; 
    ...
    switch(inst->opcodeNo){
        ...
        case DIV: ALUEmulator(inputA,inputB,OP_DIVIDE,cpu); break;
